...
The [<code class="language-javascript"><span class="token function">getNetworkLevel</span><span class="token punctuation">(</span><span class="token punctuation">)</span></code>](/api/#getnetworklevel) helper calculates the network level from either a [Player object](/api/interfaces/components.schemas.player/) or a network exp value.
