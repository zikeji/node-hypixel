...
The [<code class="language-javascript"><span class="token function">getBedwarsLevelInfo</span><span class="token punctuation">(</span><span class="token punctuation">)</span></code>](/api/#getbedwarslevelinfo) helper allows you to easily extract Bedwars level & prestige info from the [`/api/player`](/api/classes/player/#uuid) endpoint. If provided a player object, will throw an error if no Bedwars stats are found.
